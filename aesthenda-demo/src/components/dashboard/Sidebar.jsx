...
// Icons for the sidebar (using heroicons)
const CalendarIcon = () => (
...
const Sidebar = () => {
  ...
  const { currentTenant, getPrimaryColor } = useTenant();
  ...
  const getNavItems = () => {
    ...
    // Appointment management (for all roles)
    items.push({
      name: 'Appointments',
    ...
    // Staff Schedule - for staff and admin
    ...
    // Services management - admin and some staff
    if (hasRole(['salon_admin', 'salon_staff'])) {
      items.push({
        name: 'Services',
    ...
    // Clients - available to most roles
    items.push({
      name: 'Clients',
